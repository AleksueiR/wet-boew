...
* Multimedia player
*/
/*global jQuery: false, mejs, MediaElement*/
/*jshint smarttabs:true, newcap: false */
(function ($) {
	"use strict";
	var _pe = window.pe || {
		fn: {}
	...
	_pe.fn.multimedia = {
		...
		_exec: function (elm) {
			...
			if (media.get(0).error === null && media.get(0).currentSrc !== '' && media.get(0).currentSrc !== undefined) {
				canPlay = true;
				setupPlayerControls();
			} else if (media.find('source').filter('[type="' + fbVideoYTType + '"]').length > 0) {
				canPlay = true;

				// path to the flash and silverlight plugins
				mejs.MediaElementDefaults.pluginPath = _pe.add.liblocation + 'binary/';
				MediaElement(media_id, {
					success:
						function (me) {
							media = $("#" + me.id + "_container");
							setupPlayerControls(me);
						}
				});
			...
			function setupPlayerControls(mediaElement) {
				var playerNode;
				...
				if (canPlay) {
					if (mediaElement) {
						...
						//Add the interface
						$.extend(e, { object: mediaElement, evtmgr: evtmgr }, _pe.fn.multimedia._intf);

						playerNode = $("#" + mediaElement.id).get(0);
					} else {
					...
					_pe.window.on('resize', { 'media': media, playerNode: mediaElement, ratio: height / width }, function (e) {
						var h = e.data.media.parent().width() * e.data.ratio,
							w = e.data.media.parent().width();
						if (e.data.playerNode) {

							// resize player's container; needed for mobile browsers
							e.data.media.height(h);
							e.data.media.width(w);

							// resize media object
							e.data.playerNode.setVideoSize(w, h);
		...
		_intf: {
			...
			setCurrentTime: function (t) {
				if (this.object.setCurrentTime !== undefined) {
					this.object.setCurrentTime(t);
					// timeupdate is not fired if youtube video is paused
					$(this.evtmgr).trigger('timeupdate');
				} else {
			...
			setMuted: function (m) {
				if (this.object.setMuted !== undefined) {
					this.object.setMuted(m);
				} else {
					this.object.muted = m;
				}
			},
